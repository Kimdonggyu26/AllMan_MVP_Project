   ...
   <script>
      ...
      <ul class="pagination d-flex justify-content-center text-dark" id="page">
         <li class='page-item <%= pi.getCurrentPage() == 1 ? "disabled" : "" %>'>
            <a class="page-link" href="<%= contextPath%>/list.HHAL?page=<%= pi.getCurrentPage() -1 %>">&lt;</a>
         </li>
         
         <% for(int p=pi.getStartPage(); p<=pi.getEndPage(); p++) { %>
         <li class='page-item <%= p == pi.getCurrentPage() ? "active" : "" %>'>
            <a class="page-link" style="color: #ffffff;" href="<%= contextPath%>/list.HHAL?page=<%= p %>"><%= p %></a>
         </li>
         <% } %>
         ...
         function performSearch(page) {
            ...
            // Ajax 요청
            $.ajax({
               url: '<%= contextPath %>/hhal.mv',  // 서버의 검색 엔드포인트
               type: 'GET',
               data: {
